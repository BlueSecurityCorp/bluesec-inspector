...
**BlueSec Inspector**는 보안 연구원과 웹 개발자를 위한 Chrome MV3 확장 도구입니다. Chrome DevTools 창을 직접 열기 어려운 제한된 개발 환경에서, Chrome 공식 `chrome.debugger` API와 Chrome DevTools Protocol(CDP)을 사용해 모든 일반 `http/https` 웹 페이지의 Console과 Elements 중심 워크플로를 제공합니다.
...
- `http://*/*`, `https://*/*` host permission을 통해 일반 웹사이트 attach 지원
- Chrome DevTools Protocol(CDP) Runtime, Console, Log, DOM, CSS, Overlay 도메인 사용
- 일반 `http://` 및 `https://` 웹 페이지 attach 지원
- Chrome 내부 페이지, extension 페이지, `file://` 등 비웹 URL은 제한
- Console tab:
...
1. 검사하려는 `http://` 또는 `https://` 페이지를 엽니다.
2. BlueSec Inspector 확장 아이콘을 클릭합니다.
3. Chrome side panel이 열립니다.
4. `Attach`를 클릭합니다.
5. Console 또는 Elements 탭을 사용합니다.
6. 별도 창이 필요하면 `Open Window`를 클릭합니다.
...
`debugger` 권한은 attach된 탭을 Chrome DevTools Protocol로 검사하고 제어할 수 있는 강력한 권한입니다. BlueSec Inspector는 사용자가 직접 `Attach`를 누른 탭에만 debugger session을 연결합니다.
...
- Chrome 또는 로컬 보안 제품이 `chrome.debugger` attach 자체를 차단하는 경우 이 확장은 우회하지 못합니다.
...
- `Only http and https pages can be inspected.`: Chrome 내부 페이지나 `file://` 페이지가 아닌 일반 웹 페이지에서 실행합니다.

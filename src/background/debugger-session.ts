...
export class DebuggerSessionManager {
  ...
  async attach(tabId: number): Promise<void> {
    ...
    try {
      this.sessions.set(tabId, { tabId, attached: true });
      ...
      this.sessions.set(tabId, { tabId, attached: true, url: tab.url, title: tab.title });
      this.broadcast({ type: 'ATTACHED', tabId });
    } catch (error) {
      await chrome.debugger.detach(target).catch(() => undefined);
      this.sessions.set(tabId, { tabId, attached: false, error: error instanceof Error ? error.message : 'Attach failed.' });
      throw error;
    }
  }
  ...
  async detach(tabId: number, reason?: string): Promise<void> {
    ...
    this.sessions.set(tabId, { tabId, attached: false, url: previous?.url, title: previous?.title, error: reason });
